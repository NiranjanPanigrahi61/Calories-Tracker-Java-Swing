...
public class Part1 extends JFrame implements ActionListener {
    ...
    Part1() {
        ...
        healthStatusLabel.setBounds(10, 650, 800, 30);
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == addButton) {
            ...
            boolean yes=workoutYesRadioButton.isSelected();
            if(yes == true){
                tfExerciseTime.setEditable(true);
            }else{
                tfExerciseTime.setText(String.valueOf(0));
            ...
            int showProgress=progressValue;
            progressBar.setValue(progressValue);
            progressBar.setString(String.format("Today's Calories Lost : %d%% ",showProgress));

            // Calculate and update weight loss progress bar
            double weight = Double.parseDouble(weightField.getText());
            double weightLoss = calculateWeightLoss(caloriesLost, weight);
            int weightLossProgressValue=0;
            if(weightLoss == 0.0){
                weightLossProgressValue = (int)weight + caloriesGained;
            }else{
                weightLossProgressValue = (int) ((weightLoss / weight) * 100);
            }
            weightLossProgressBar.setValue(weightLossProgressValue);
            weightLossProgressBar.setString(String.format("Weight : %.2f kg", weightLoss));
    ...
    private int calculateCaloriesLost() {
        ...
        int caloriesLost=exerciseTime * 5; //= exerciseTime * 5; // Assuming 5 calories burned per minute of exercise
        if (didWorkout) {
            caloriesLost += 100 ; // Additional calories burned for workout
        }
        return caloriesLost;
    }
    ...
    private double calculateWeightLoss(int caloriesLost, double weight) {
        ...
        double weightInCalories = weight*7700;
        if(caloriesLost <= 0){
            return (weightInCalories + weight) / 7700.0;
        }
        return (weightInCalories-caloriesLost) / 7700.0;
    }
